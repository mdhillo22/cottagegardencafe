...
router.get('/:recordid/show', function(req, res, next) {
    let query = "SELECT product_id, productname, prodimage, description, ingredients, category_id, saleprice, status, homepage FROM product WHERE product_id = " + req.params.recordid;
    // execute query
    db.query(query, (err, result) => {
    if (err) {
        console.log(err);
...
router.get('/addrecord', adminonly, function(req, res, next) {
    ...
    res.render('product/addrec', {categories: categories});
    }); 
});   
...
router.post('/', adminonly, function(req, res, next) {
    ...
    if (req.body.homepage) { //if homepage checked
        homepage_value = 1; 
    }

    let insertquery = "INSERT INTO product (productname, prodimage, description, ingredients, category_id, saleprice, status, homepage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    db.query(insertquery,[req.body.productname, req.body.prodimage, req.body.description,
    req.body.ingredients, req.body.category_id, req.body.saleprice, req.body.status, homepage_value],(err, result) => {
    if (err) {
        console.log(err);
        res.render('error');
    } else {
...
router.get('/:recordid/edit', adminonly, function(req, res, next) {
    let query = "SELECT product_id, productname,prodimage, description, ingredients, category_id, saleprice, status, homepage FROM product WHERE product_id = "
    + req.params.recordid;
    // execute query
    db.query(query, (err, result) => {
    if (err) {
...
router.post('/save', adminonly, function(req, res, next) {
    ...
    let updatequery = "UPDATE product SET productname = ?, prodimage = ?, description = ?, ingredients = ?, category_id = ?, saleprice = ?, status = ?, homepage = ? WHERE product_id = " + req.body.product_id;
